...
VOXOWL_HOST const char* strVoxelFormat( voxel_format_t f );
...
VOXOWL_HOST void voxelmapCreate( voxelmap_t *, voxel_format_t, ivec3_32_t size );
...
VOXOWL_HOST_AND_DEVICE ivec3_32_t blockCount( voxel_format_t f, ivec3_32_t size );
...
VOXOWL_HOST_AND_DEVICE ivec3_32_t blockPosition( voxel_format_t f, ivec3_32_t position );

/*! Returns the 1D offset within a block to the voxel in `position`, given format f 
   Column-major (z) indexing is utilized */
VOXOWL_HOST_AND_DEVICE unsigned int blockOffset( voxel_format_t f, ivec3_32_t position );

/*! Return the size of a volume's data in bytes */
VOXOWL_HOST_AND_DEVICE size_t voxelmapSize( voxelmap_t * );
VOXOWL_HOST_AND_DEVICE size_t voxelmapSize( voxel_format_t f, ivec3_32_t size, ivec3_32_t scale =ivec3_32(1) );

/*! Access an array based volume by coordinates. Returns a pointer to the block containing the element */
VOXOWL_HOST_AND_DEVICE void* voxel( voxelmap_t*, ivec3_32_t position );
...
VOXOWL_HOST_AND_DEVICE glm::vec4 unpackABGR_UINT32( uint32_t rgba );
