...
//
// Minor changes to work with CUDA by Micky Faas
// Only the basic C functions work with CUDA, without assemly-level optimizations
//
...
/* Begin adaptations for CUDA */

#if __GNUC__ || __has_builtin(__builtin_clzll)
#define LIBDIVIDE_BUILTIN_CLZ
#endif

#if defined(__CUDACC__)
#define LIBDIVIDE_API __host__ __device__
#define LIBDIVIDE_PRIVATE __host__ __device__
#else
#define LIBDIVIDE_PRIVATE
#endif

#if defined(__CUDA_ARCH__)
#undef HAS_INT128_T
#undef LIBDIVIDE_IS_I386
#undef LIBDIVIDE_IS_X86_64
#undef LIBDIVIDE_USE_SSE2
#undef LIBDIVIDE_USE_SSE4_1
#undef LIBDIVIDE_BUILTIN_CLZ
#endif

/* End adaptations for CUDA */
...
LIBDIVIDE_PRIVATE static inline uint32_t libdivide__mullhi_u32(uint32_t x, uint32_t y) {
    uint64_t xl = x, yl = y;
    uint64_t rl = xl * yl;
    return (uint32_t)(rl >> 32);
}
 
LIBDIVIDE_PRIVATE static uint64_t libdivide__mullhi_u64(uint64_t x, uint64_t y) {
#if HAS_INT128_T
...
LIBDIVIDE_PRIVATE static inline int64_t libdivide__mullhi_s64(int64_t x, int64_t y) {
#if HAS_INT128_T
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide__u64_to_m128(uint64_t x) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_get_FFFFFFFF00000000(void) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_get_00000000FFFFFFFF(void) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_s64_signbits(__m128i v) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_u32_to_m128i(uint32_t amt) {
    return _mm_set_epi32(0, 0, 0, amt);
}
    
LIBDIVIDE_PRIVATE static inline __m128i libdivide_s64_shift_right_vector(__m128i v, int amt) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide__mullhi_u32_flat_vector(__m128i a, __m128i b) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_mullhi_u64_flat_vector(__m128i x, __m128i y) {
    // full 128 bits are x0 * y0 + (x0 * y1 << 32) + (x1 * y0 << 32) + (x1 * y1 << 64)
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_mullhi_s64_flat_vector(__m128i x, __m128i y) {
...
LIBDIVIDE_PRIVATE static inline __m128i libdivide_mullhi_s32_flat_vector(__m128i a, __m128i b) {
...
LIBDIVIDE_PRIVATE static inline int32_t libdivide__count_trailing_zeros32(uint32_t val) {
...
LIBDIVIDE_PRIVATE static inline int32_t libdivide__count_trailing_zeros64(uint64_t val) {
...
LIBDIVIDE_PRIVATE static inline int32_t libdivide__count_leading_zeros32(uint32_t val) {
#if defined( LIBDIVIDE_BUILTIN_CLZ )
...
LIBDIVIDE_PRIVATE static inline int32_t libdivide__count_leading_zeros64(uint64_t val) {
#if defined( LIBDIVIDE_BUILTIN_CLZ )
    // Fast way to count leading zeros
...
LIBDIVIDE_PRIVATE static uint32_t libdivide_64_div_32_to_32(uint32_t u1, uint32_t u0, uint32_t v, uint32_t *r) {
    uint32_t result;
    __asm__("divl %[v]"
            : "=a"(result), "=d"(*r)
            : [v] "r"(v), "a"(u0), "d"(u1)
...
LIBDIVIDE_PRIVATE static uint64_t libdivide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t v, uint64_t *r) {
    // u0 -> rax
    // u1 -> rdx
    // divq
    uint64_t result;
...
LIBDIVIDE_PRIVATE static inline void libdivide_u128_shift(uint64_t *u1, uint64_t *u0, int32_t signed_shift)
{
    if (signed_shift > 0) {
...
LIBDIVIDE_PRIVATE static inline struct libdivide_u32_t libdivide_internal_u32_gen(uint32_t d, int branchfree) {
...
LIBDIVIDE_PRIVATE static inline struct libdivide_u64_t libdivide_internal_u64_gen(uint64_t d, int branchfree) {
    // 1 is not supported with branchfree algorithm
    LIBDIVIDE_ASSERT(!branchfree || d != 1);
...
LIBDIVIDE_PRIVATE static inline int32_t libdivide__mullhi_s32(int32_t x, int32_t y) {
    int64_t xl = x, yl = y;
    int64_t rl = xl * yl;
    return (int32_t)(rl >> 32); // needs to be arithmetic shift
}

LIBDIVIDE_PRIVATE static inline struct libdivide_s32_t libdivide_internal_s32_gen(int32_t d, int branchfree) {
...
LIBDIVIDE_PRIVATE static inline struct libdivide_s64_t libdivide_internal_s64_gen(int64_t d, int branchfree) {
    LIBDIVIDE_ASSERT(!branchfree || (d != 1 && d != -1));
